...
- **Weekly Summary Template**: `<WORKSPACE_PATH>/Templates/Weekly Summary Template.md`
- **Weekly Summaries Folder**: `<WORKSPACE_PATH>/Weekly Summaries/`
- **Progress Log**: `<WORKSPACE_PATH>/Progress Log.md`
...
```bash
git -C <WORKSPACE_PATH> diff --staged --name-status
```
...
3. Check for system file changes. A system file is any staged path matching:
   ...
   - Read `<WORKSPACE_PATH>/Agent-Change-Log.md`
...
3. Commit and push:
```bash
git -C <WORKSPACE_PATH> commit -m "<generated message>"
