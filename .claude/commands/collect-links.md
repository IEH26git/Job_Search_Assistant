...
- **Links File**: `$PWD/REF/Links.md`
- **Email Queue**: `$PWD/.claude/links_email_queue.md`
...
Read `$PWD/REF/Links.md` if it exists.
...
Read the current contents of `$PWD/REF/Links.md` (or start with an empty file if it doesn't exist).
...
If there are no new entries to add (all URLs were duplicates or all emails had no usable URLs), tell the user and stop — do not write anything.

---

### Step 7: Save the email queue and report

**7a. Save the email queue.** Write (or overwrite) `$PWD/.claude/links_email_queue.md` with one row per processed email — all emails from this run.
...
**7b. Report to the user.** Tell the user:
...
Read `$PWD/.claude/links_email_queue.md`.
...
**10a.** Overwrite `$PWD/.claude/links_email_queue.md` with:
