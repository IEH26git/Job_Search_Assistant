...
- **Role Criteria**: `$PWD/Templates/Role_Search_Criteria.md`
- **JDs Folder**: `$PWD/Resume/JDs/`
- **Task Board**: `$PWD/Task Board.md`
- **Email Queue**: `$PWD/.claude/email_queue.md`
...
Read `$PWD/Templates/Role_Search_Criteria.md`.
...
2. Write the file to `$PWD/Resume/JDs/<filename>` with this format:
...
Write the file to `$PWD/Resume/JDs/Manual-Review-YYYY-MM-DD.csv` with these columns:
...
Write the file to `$PWD/Resume/JDs/Manual-Review-YYYY-MM-DD.md` using this format:
...
**6c. Save the email queue.** Write (or overwrite) `$PWD/.claude/email_queue.md` with one row per processed email — all emails from this run, not just those that created JD files.
...
**6d. Update the Task Board.** Read `$PWD/Task Board.md` and append one line to the **Inbox** section:
...
Read `$PWD/.claude/email_queue.md`.
...
**9a.** Overwrite `$PWD/.claude/email_queue.md` with:
